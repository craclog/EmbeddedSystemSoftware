...
void mode4_blink(){
	int i;
    // printf("cur_visible:%d\n", cur_visible);
	if(cur_visible == HIDE_ON_BUSH){
		// Faker
	} else if(cur_visible == NO_HIDE){ // blink
		// send dot msg per 1 sec
		gettimeofday(&m4_cur_time, NULL);
		if(m4_cur_time.tv_sec >= next_blink_time.tv_sec){
			unsigned char tmp[10];
			for(i=0; i< DOT_HEIGHT; i++)
				tmp[i] = hex_dot[i];
			next_blink_time.tv_sec = m4_cur_time.tv_sec + 1;
